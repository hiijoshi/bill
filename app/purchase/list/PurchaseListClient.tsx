...
export default function PurchaseListClient({
  ...
  const hasInitialBills = Boolean(initialCompanyId)
  const { financialYear } = useClientFinancialYear({
    initialPayload: initialLayoutData?.financialYearPayload || undefined
  })
  const [purchaseBills, setPurchaseBills] = useState<PurchaseBill[]>(initialBills)
  const [loading, setLoading] = useState(!hasInitialBills)
  ...
  useEffect(() => {
    ...
    setPurchaseBills(initialBills)
    setLoading(false)
    setClientCache(`purchase-bills:${initialCompanyId}`, initialBills)
  ...
  }, [fetchPurchaseBills, hasInitialBills])
  ...
  if (loading && !companyId && !hasPurchaseData) {
    return (
      <AppLoaderShell
        kind="purchase"
        companyId={companyId}
        title="Opening purchase list"
        message="Collecting purchase bills, marka values, filters, and print selections."
      />
    )
  }
