...
export default function PurchasePrintClient({ printData }: Props) {
  ...
  const returnPath = useMemo(() => {
    if (returnTo !== 'entry') return null
    return companyId
      ? `/purchase/entry?companyId=${encodeURIComponent(companyId)}`
      : '/purchase/entry'
  }, [companyId, returnTo])
  ...
  return (
    <div className="print-root bg-white text-black p-4 print:p-0">
      ...
      <PurchasePrintSheet printData={printData} />
    </div>
  )
}

export function PurchasePrintSheet({ printData }: Props) {
  const payableToFarmer = Math.max(0, printData.amount)
