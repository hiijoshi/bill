...
import { loadClientPermissions, primeClientPermissions, type ClientPermissionsPayload } from '@/lib/client-permissions'
...
interface MainDashboardClientProps {
  ...
  initialPrimaryCompanyId?: string
  initialDashboardData?: DashboardPayload
  initialHasDashboardAccess?: boolean
  initialDashboardAccessResolved?: boolean
  initialPermissionPayload?: ClientPermissionsPayload | null
...
export default function MainDashboardClient({
  ...
  initialPrimaryCompanyId = '',
  initialDashboardData = emptyDashboardPayload,
  initialHasDashboardAccess = false,
  initialDashboardAccessResolved = false,
  initialPermissionPayload = null,
  initialLayoutData = null
}: MainDashboardClientProps) {
  const router = useRouter()
  const [activeTab, setActiveTab] = useState<ActiveTab>('purchase')
  const [loading, setLoading] = useState(!initialDashboardAccessResolved)
  const [data, setData] = useState<DashboardPayload>(initialDashboardData)
  ...
  const [primaryCompanyId, setPrimaryCompanyId] = useState<string>(initialPrimaryCompanyId)
  const [dashboardAccessResolved, setDashboardAccessResolved] = useState(initialDashboardAccessResolved)
  const [hasDashboardAccess, setHasDashboardAccess] = useState(initialHasDashboardAccess)
  const [uiMessage, setUiMessage] = useState<string | null>(null)
  const [uiError, setUiError] = useState<string | null>(null)
  const [fetchFailures, setFetchFailures] = useState<string[]>([])
  const [reloadNonce, setReloadNonce] = useState(0)
  const skipPreparedBootstrapRef = useRef(initialDashboardAccessResolved)
  const skipPreparedDataFetchRef = useRef(
    initialDashboardAccessResolved && initialHasDashboardAccess && initialSelectedCompanyIds.length > 0
  )
  ...
  useEffect(() => {
    if (initialCompanies.length > 0) {
      setClientCache(COMPANIES_CACHE_KEY, initialCompanies, { persist: true })
    }
    if (initialPrimaryCompanyId) {
      setClientCache(ACTIVE_COMPANY_CACHE_KEY, initialPrimaryCompanyId, { persist: true })
    }
    if (initialPermissionPayload) {
      primeClientPermissions(initialPermissionPayload)
    }
    if (initialSelectedCompanyIds.length > 0) {
      setClientCache(buildDashboardCacheKey(initialSelectedCompanyIds), initialDashboardData, { persist: true })
    }
  }, [
    initialCompanies,
  ...
  useEffect(() => {
    if (reloadNonce === 0 && skipPreparedBootstrapRef.current) {
      skipPreparedBootstrapRef.current = false
      setLoading(false)
      return
    }
  ...
  useEffect(() => {
    ...
    if (skipPreparedDataFetchRef.current) {
      skipPreparedDataFetchRef.current = false
      return
    }
  ...
  if (loading && !hasDashboardData) {
    return (
      <DashboardLayout companyId={primaryCompanyId} initialData={initialLayoutData}>
