...
export default function BankMasterPage() {
  ...
  const handleDeleteAll = async () => {
    ...
    const response = await fetch('/api/banks?all=true', { method: 'DELETE' })
  ...
  const handleImportCsv = async (file: File) => {
    const { ok, result } = await uploadMasterCsv('/api/banks/import', file)
    if (!ok) {
      alert(result.error || 'Bank import failed')
      return
    }

    alert(formatMasterImportSummary('Bank', result))
    await fetchBanks()
  }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-2 md:items-end">
              ...
              <MasterCsvTemplateHint templateKey="bank" />
            </div>
          </div>
