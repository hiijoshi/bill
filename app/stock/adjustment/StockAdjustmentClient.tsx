...
import { getDefaultTransactionDateInput, type ClientFinancialYearPayload } from '@/lib/client-financial-years'
...
interface StockAdjustmentClientProps {
  ...
  initialStockSummary?: ProductMetrics[]
  initialStockLedger?: StockLedgerEntry[]
...
export default function StockAdjustmentClient({
  ...
  const [pageLoading, setPageLoading] = useState(!hasInitialWorkspace)
  const [refreshing, setRefreshing] = useState(false)
  const [errorMessage, setErrorMessage] = useState<string | null>(null)
  const [submitting, setSubmitting] = useState(false)
  const [products, setProducts] = useState<Product[]>(initialProducts)
  const [stockSummary, setStockSummary] = useState<ProductMetrics[]>(initialStockSummary)
  ...
  useEffect(() => {
    ...
    setSelectedProduct((current) => {
      if (current && initialProducts.some((product) => product.id === current)) {
        return current
      }
      return initialProducts.find((product) => product.currentStock > 0)?.id || initialProducts[0]?.id || ''
    })
    setPageLoading(false)
    setClientCache(`stock-overview:${initialCompanyId}`, {
      products: initialProducts,
      summary: initialStockSummary,
      recentEntries: initialStockLedger
    })
  }, [
    initialCompanyId,
    initialProducts,
  ...
  }, [fetchStockContext, hasInitialWorkspace, router])
